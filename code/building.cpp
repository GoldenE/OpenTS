 ...
 *=============================================================================================*/
RadioMessageType BuildingClass::Receive_Message(RadioClass * from, RadioMessageType message, RadioParameter & param)
{
	switch (message) {
		...
		case RADIO_DOCKING: {
			...
			*/
			...
			if (Class->IsHospital || Class->IsArmory) {
				if (Contact_With_Whom() != from) {
					...
					param = reinterpret_cast<RadioParameter>(&Map[Get_Coord()]);
			...
			*/
			...
			if (Transmit_Message(RADIO_NEED_TO_MOVE) == RADIO_ROGER || needs_to_move) {
				param = reinterpret_cast<RadioParameter>(this);
				if (Class->IsDockUnload || Class->IsWeeder) {
					param = reinterpret_cast<RadioParameter>(&Map[Get_Cell() + Cell(2, 1)]);
