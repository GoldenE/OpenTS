...
long __cdecl Disk_VQA_Stream_Handler(VQAHandle *vqa, long action, void *buffer, long nbytes)
{
	...
	switch (action) {
		 ...
		 */
		case VQACMD_SEEK:
			error = (lseek(fh, nbytes, static_cast<int>(reinterpret_cast<std::intptr_t>(buffer))) == -1);
			break;

		case VQACMD_SEEKPEEK:
			if (nbytes > 0) {
				error = lseek(fh, nbytes - 1, static_cast<int>(reinterpret_cast<std::intptr_t>(buffer))) == -1;
				if (error == 0) {
