...
std::intptr_t __cdecl VQA_Memory_Handler(VQAHandle *vqa, long action, void *buffer, long nbytes);
...
long User_Update(VQAHandle *vqa)
{
	...
	if (config->EventHandler != NULL) {
		if (curframe->Flags & VQAFRMF_LOOPED) {
			config->EventHandler((VQAHandle *)vqap, VQAEVENT_LOOPED, reinterpret_cast<void *>(static_cast<std::intptr_t>(curframe->FrameNum)), vqap->LoopID);
			curframe->Flags &= ~VQAFRMF_LOOPED;
		}
		if (curframe->Flags & VQAFRMF_LOOPJMP) {
			config->EventHandler((VQAHandle *)vqap, VQAEVENT_LOOPJUMP, reinterpret_cast<void *>(static_cast<std::intptr_t>(curframe->FrameNum)), vqap->LoopID);
