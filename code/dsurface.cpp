		...
		if (abs_dz > 0) {
			...
			for (int i = abs_dz; i != 0; i--) {
				...
				if (xdelta > 0) {
					...
					zbuffer += zwidth;
					if (zwidth > 0) {
						zbuffer = (unsigned short *)DepthBuffer->Wrap_Overflow((std::uintptr_t)zbuffer);
					} else {
						zbuffer = (unsigned short *)DepthBuffer->Wrap_Underflow((std::uintptr_t)zbuffer);
					}
					z += zwrap;
				}
				...
				if (dx > 0) {
					...
					do {
						...
						if (otherweight > 256) {
							...
							abuffer += zwidth;
							if (zwidth > 0) {
								abuffer = (unsigned short *)AlphaBuffer->Wrap_Overflow((std::uintptr_t)abuffer);
							} else {
								abuffer = (unsigned short *)AlphaBuffer->Wrap_Underflow((std::uintptr_t)abuffer);
							}
							z += zwrap;
						}
