...
inline bool PriorityQueueClass<T>::Insert(T & node)
{
	...
	if (reinterpret_cast<std::uintptr_t>(&node) > MaxNodePointer) {
		MaxNodePointer = reinterpret_cast<std::uintptr_t>(&node);
	}

	if (reinterpret_cast<std::uintptr_t>(&node) < MinNodePointer) {
		MinNodePointer = reinterpret_cast<std::uintptr_t>(&node);
	}
...
void PriorityQueueClass<T>::Serialize(S & stream, T * nodes)
{
	...
	for (int slot = 0; slot < Size; slot++) {
		...
		if (stream.Is_Loading()) {
			...
			if (reinterpret_cast<std::uintptr_t>(Heap[slot]) > MaxNodePointer) {
				MaxNodePointer = reinterpret_cast<std::uintptr_t>(Heap[slot]);
			}
			if (reinterpret_cast<std::uintptr_t>(Heap[slot]) < MinNodePointer) {
				MinNodePointer = reinterpret_cast<std::uintptr_t>(Heap[slot]);
			}
		}
	}
}
