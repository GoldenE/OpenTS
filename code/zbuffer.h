...
class ZBuffer
{
	...
	public:
		...
		void Set(std::uintptr_t dst, int size, unsigned short value);
		...
		std::uintptr_t Get_Buffer_End(void) const { return(BufferEnd); }
	...
	private:
		 ...
		 */
		std::uintptr_t BufferStart;
		std::uintptr_t BufferEnd;
		std::uintptr_t BufferSize;
...
inline std::uintptr_t ZBuffer::Wrap_Underflow(std::uintptr_t position) const
{
...
{
	return((unsigned short *)DepthBuffer->Wrap_Overflow((std::uintptr_t)buf));
}
