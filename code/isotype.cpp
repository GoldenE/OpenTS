...
/// This routine reads the tile set into memory, retains its relative record offsets,
...
// Scratch state for the C++ iso-tile rasterizer in IsometricTileTypeClass::Draw_Tile.
///
...
#pragma pack(push, 1)
struct IsoBlitState {
	std::uintptr_t SrcPixel;
	std::uintptr_t SrcDepth;
		...
		if (surface.Bytes_Per_Pixel() == 2) {
			...
			if (set != NULL) {
				if (use_z) {
					...
					IsoDrawData.BaseDepth = IsoDrawData.BaseDepth + height * set->Height / -2;
				}
				...
				if (!clipped_out) {
					...
					if (IsoDrawData.DestPtr == NULL) {
						...
						IsoDrawData.SrcPixel = (std::uintptr_t)(record + 1);
						...
						if (use_z && record->IsHasZData) {
							IsoDrawData.SrcDepth = (std::uintptr_t)record + record->ZDataOffset;
						}
						...
						if (use_z) {
							...
							if ((std::uintptr_t)&IsoDrawData.DepthPtr[IsoDrawData.SpanWidth + 2 + IsoDrawData.SpanHeight * DepthBuffer->BufferWidth] >= DepthBuffer->Get_Buffer_End()) {
								if (fill) {
									...
									for (int row = 0; row < IsoDrawData.SpanHeight; ++row) {
										...
										IsoDrawData.DepthPtr = (unsigned short *)DepthBuffer->Wrap_Overflow((std::uintptr_t)IsoDrawData.DepthPtr);
									...
									if (!Rule->IsBlendedFog) {
										...
										for (int row = 0; row < IsoDrawData.SpanHeight; ++row) {
											...
											IsoDrawData.SrcPixel = (std::uintptr_t)IsoDrawData.ImageBase + rowoff;
											int start = *IsoDrawData.RowStartCol * 2;
											IsoDrawData.RowDest = (unsigned short *)destrow;
											IsoDrawData.RowDepth = zrow;
										...
										unsigned char * destrow = (unsigned char *)IsoDrawData.DestPtr;
										for (int row = 0; row < IsoDrawData.SpanHeight; ++row) {
											IsoDrawData.SrcPixel = (std::uintptr_t)IsoDrawData.ImageBase + *IsoDrawData.RowSrcOffset;
											int start = *IsoDrawData.RowStartCol * 2;
											IsoDrawData.RowDest = (unsigned short *)destrow;
											IsoDrawData.RowDepth = zrow;
											IsoDrawData.DestPtr = (unsigned short *)&destrow[start];
									...
									for (int row = 0; row < IsoDrawData.SpanHeight; ++row) {
										unsigned short rowoff = *IsoDrawData.RowSrcOffset;
										IsoDrawData.SrcPixel = (std::uintptr_t)IsoDrawData.ImageBase + rowoff;
										IsoDrawData.SrcDepth = (std::uintptr_t)IsoDrawData.DepthBase + rowoff;
										...
										IsoDrawData.AlphaPtr = (unsigned short *)AlphaBuffer->Wrap_Overflow((std::uintptr_t)IsoDrawData.AlphaPtr);
										int run = *IsoDrawData.RowRunLength;
										if (run > 0) {
											do {
							...
							if ((std::uintptr_t)&arow[IsoDrawData.SpanWidth + 2 + IsoDrawData.SpanHeight * AlphaBuffer->Get_Buffer_Width()] >= AlphaBuffer->Get_Buffer_End()) {
								unsigned char * destrow = (unsigned char *)IsoDrawData.DestPtr;
								for (int row = 0; row < IsoDrawData.SpanHeight; ++row) {
									IsoDrawData.SrcPixel = (std::uintptr_t)IsoDrawData.ImageBase + *IsoDrawData.RowSrcOffset;
									int startcol = *IsoDrawData.RowStartCol * 2;
				...
				if (!skip_extra) {
					if (record->IsHasExtraData && !fill && !depth_only && !fog) {
						...
						if (extra_ok) {
							...
							if (IsoDrawData.DestPtr != NULL) {
								IsoDrawData.SrcPixel = (std::uintptr_t)record + record->ExtraOffset + IsoDrawData.ClipLeft;
								IsoDrawData.SrcPixel = IsoDrawData.ClipTop * record->ExtraWidth + IsoDrawData.SrcPixel;
								if (use_z) {
									IsoDrawData.SrcDepth = (std::uintptr_t)record + record->ExtraZOffset + IsoDrawData.ClipLeft;
								...
								} else if ((std::uintptr_t)&IsoDrawData.AlphaPtr[IsoDrawData.SpanWidth + 2 + IsoDrawData.SpanHeight * AlphaBuffer->Get_Buffer_Width()] >= AlphaBuffer->Get_Buffer_End()) {
