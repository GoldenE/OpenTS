...
item_data TreeView_get_item_data(DWORD xy)
{
	...
	if (reinterpret_cast<LONG_PTR>(hwnd1) == GetWindowLongPtr(TreeView_LastHandle, GWLP_HWNDPARENT)) {
		hwnd2 = hwnd1;
	}
...
BOOL On_WM_CONTEXTMENU(WPARAM window)
{
	...
	return(WinHelp((HWND)window, "SUN.HLP", HELP_CONTEXTMENU, reinterpret_cast<ULONG_PTR>(identifiers)));
}
...
void Prefetch_Audio_Buffer(char *src, int size)
{
	...
	int n1 = static_cast<int>(reinterpret_cast<uintptr_t>(src) % AUD_FILL_SIZE);
	int n2 = size - n1;
	char *ptr = src + n1;
