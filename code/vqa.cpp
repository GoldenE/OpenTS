...
std::intptr_t __cdecl VQAEventHandler(VQAHandle * vqa, long action, void * buffer, long nbytes);
std::intptr_t __cdecl VQAMemoryHandler(VQAHandle * vqa, long action, void * buffer, long nbytes);
...
long VQAClass::CacheHandler(long action, void * buffer, long nbytes)
{
	...
	switch (action) {
		...
		case VQACMD_SEEK:
			switch (reinterpret_cast<std::intptr_t>(buffer)) {
				case 1:
					Cache.file_buffer_pos += nbytes;
					rc = 0;
					break;
...
long VQAClass::CCFileHandler(long action, void * buffer, long nbytes)
{
	...
	*/
	switch (action) {
		...
		*/
		case VQACMD_SEEK:
			error = (FileHandle.Seek(nbytes, static_cast<int>(reinterpret_cast<std::intptr_t>(buffer))) == 0);
			break;

		case VQACMD_SEEKPEEK:
			if (nbytes > 0) {
				error = FileHandle.Seek(nbytes - sizeof(tmp), static_cast<int>(reinterpret_cast<std::intptr_t>(buffer))) == 0;
				if (error == 0) {
...
std::intptr_t __cdecl VQAMemoryHandler(VQAHandle * vqa, long action, void * buffer, long nbytes)
{
	std::intptr_t error = 0;

	switch (action) {

		case VQAMEM_ALLOC:
			error = reinterpret_cast<std::intptr_t>(malloc(nbytes));
			break;
		...
		case VQAMEM_LOCK:
		case VQAMEM_UNLOCK:
			error = reinterpret_cast<std::intptr_t>(buffer);
			break;
...
std::intptr_t __cdecl VQAEventHandler(VQAHandle * vqa, long action, void * buffer, long nbytes)
{
	...
	switch (action) {
		...
		case VQAEVENT_LOCK:
			error = reinterpret_cast<std::intptr_t>(_this->Handle_Lock_Event());
			break;
