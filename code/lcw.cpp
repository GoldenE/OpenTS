 ...
 *             for the destination buffer is datasize + (datasize+62)/63 + 1.                 *
 *                                                                                             *
 * HISTORY:                                                                                    *
...
int LCW_Comp(void const * source, void * dest, int datasize)
{
	...
	*output++ = *input;
	...
	int position = 1;
	auto put_word = [&](unsigned value) {
		*output++ = static_cast<unsigned char>(value);
		*output++ = static_cast<unsigned char>(value >> 8);
	};
	while (position < datasize) {
		...
		int candidate = 0;
		while (true) {
			if (datasize - position > 64 && input[position] == input[position + 64]) {
				int run = 0;
				while (position + run < datasize && input[position + run] == input[position]) ++run;
				// The original scan leaves the final input byte outside an EOF run.
				if (position + run == datasize) --run;
				if (run >= 65) {
					*output++ = 0xfe;
					put_word(run);
			...
			while (candidate < position && input[candidate] != input[position]) ++candidate;
			if (candidate == position) break;
			int current = candidate++;
			if (input[position + best - 1] != input[current + best - 1]) continue;
			int count = 0;
			while (position + count < datasize && input[current + count] == input[position + count]) ++count;
			if (count >= best) {
				best = count;
				match = current;
			}
		...
		if (best <= 2) {
			...
			*output++ = input[position++];
			...
			int distance = position - match;
			if (best <= 10 && distance <= 0xfff) {
				*output++ = static_cast<unsigned char>(((best - 3) << 4) | (distance >> 8));
				*output++ = static_cast<unsigned char>(distance);
			} else {
				if (best <= 64) {
					*output++ = static_cast<unsigned char>(0xc0 | (best - 3));
				} else {
					*output++ = 0xff;
					put_word(best);
				}
