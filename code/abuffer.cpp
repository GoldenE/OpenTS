	...
	BufferHeight(rect.Height)
{
	...
	BufferStart = (std::uintptr_t)(SurfacePtr->Lock());
	SurfaceOffset = 0;
		...
		for (int i = 0; i < rect.Height; ++i) {
			for (int j = 0; j < rect.Width; ++j) {
				*surfbuffptr = *pixptr;
				...
				pixptr = (unsigned short *)Wrap_Overflow((std::uintptr_t)pixptr);
			}
			surfbuffptr += steps;
		}
	}
...
void ABuffer::Set(std::uintptr_t dst, int size, unsigned short value)
{
	/// Write a single pixel to bring the address up to an int boundary.
	if ((std::uintptr_t)dst & 2) {
		if (size != 0) {
			*(unsigned short *)dst = value;
			dst = (std::uintptr_t)((unsigned short *)dst + 1);
			size--;
		}
	}
...
void ABuffer::Pan(int x, int y, unsigned short value)
{
	...
	if (abs(x_delta) > BufferWidth || abs(y_delta) > BufferHeight) {
		...
		if (x_delta != 0) {
			...
			std::uintptr_t new_offset = Wrap_Underflow(SurfaceOffset + BufferStart);
			new_offset = Wrap_Overflow(new_offset);
			SurfaceOffset = static_cast<int>(new_offset - BufferStart);
...
void ABuffer::Update(Rect rect)
{
	...
	for (int i = 0; i < rect.Height; ++i) {
		...
		if ((buffptr + rect.Width * ABUFFER_BPP) >= BufferEnd) {
			size = static_cast<unsigned int>((BufferEnd - buffptr) / ABUFFER_BPP);
